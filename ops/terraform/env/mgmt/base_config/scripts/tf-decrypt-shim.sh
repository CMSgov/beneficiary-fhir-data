...
KMS_KEY_ARN="$(jq -r '"\(.kms_key_arn)"' <<<"$TF_QUERY")"
readonly KMS_KEY_ARN

if [[ $SEED_ENV == "null" || $KMS_KEY_ARN == "null" ]]; then
  echo "kms_key_arn or seed_env unspecified"
  exit 1
fi
...
env "${vars[@]}" "$SCRIPT_DIR"/read-and-decrypt-yaml.sh "$SEED_ENV" "$KMS_KEY_ARN"
