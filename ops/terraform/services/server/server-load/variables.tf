...
variable "server_load_sqs_queue_name" {
...
variable "server_load_node_lambda_name" {
...
variable "server_load_node_spawn_time" {
...
variable "server_load_max_spawned_nodes" {
...
variable "server_load_max_spawned_users" {
...
variable "server_load_user_spawn_rate" {
  description = "The rate at which simulated Locust users (not worker nodes) will spawn. Set this equal to server_load_max_spawned_users if all users should be spawned immediately"
  type        = number
  default     = 1
}

variable "server_load_test_runtime_limit" {
  description = "The maximum runtime for the current load test. Acts as a failsafe against runaway load testing"
  type        = string
  default     = "10m30s"
}

variable "server_load_coasting_time" {
...
variable "server_load_stop_on_node_limit" {
