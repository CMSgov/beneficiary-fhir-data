...
def get_data_set_id(client, account_id):
    data_sets = client.search_data_sets(AwsAccountId=account_id, Filters=[
        {
            'Operator': 'StringLike',
            'Name': 'DATASET_NAME',
...
def refresh_data_set(client, account_id, data_set_id):
    ...
    response = client.create_ingestion(AwsAccountId=account_id, DataSetId=data_set_id, IngestionId=ingestion_id,
                                           IngestionType='INCREMENTAL_REFRESH')
    ingestion_status = response["IngestionStatus"]
    print(f'Refresh status of dataset {data_set_id} with an ingestion id of {ingestion_id} is {ingestion_status}')
