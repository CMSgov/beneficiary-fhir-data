#!/usr/bin/env sh
#######################################
# Fetch the even/odd autoscaling group launch template versions, instance and warm_pool counts, and
# the attached Target Groups.
# Returned json object includes...
# "odd_launch_template_version", "odd_desired_capacity", "odd_min_size", "odd_warmpool_min_size",
# "odd_target_groups_csv", "even_launch_template_version", "even_desired_capacity", "even_min_size",
# and "even_warmpool_min_size", "even_target_groups_csv"  as strings.
...
yq --output-format=json --null-input '{
  ...
  "odd_target_groups_csv": env(odd) | .TargetGroupARNs // [] | join(",") | tostring,
  ...
  "even_target_groups_csv": env(even) | .TargetGroupARNs // [] | join(",") | tostring
}'
