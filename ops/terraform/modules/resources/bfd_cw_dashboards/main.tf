...
resource "aws_cloudwatch_dashboard" "bfd_server_dashboard" {
  ...
  dashboard_body = templatefile("${path.module}/templates/dashboard.tpl", { dashboard_namespace = "bfd-${var.env}/bfd-server", env = var.env })
}
...
resource "aws_cloudwatch_dashboard" "bfd_server_aws_services_dashboard" {
  ...
  dashboard_body = templatefile("${path.module}/templates/aws-services-dashboard.tpl", {
  dashboard_namespace = "bfd-${var.env}/bfd-server", env = var.env })
}
...
resource "aws_cloudwatch_dashboard" "bfd_server_opentelemetry_dashboard" {
  ...
  dashboard_body = templatefile("${path.module}/templates/opentelemetry-dashboard.tpl", { dashboard_namespace = "bfd-${var.env}/bfd-server", env = var.env })
}