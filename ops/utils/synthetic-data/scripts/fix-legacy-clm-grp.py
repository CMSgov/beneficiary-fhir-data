...
def gather_unique_grp_ids_for_legacy_synthetic_data(db_string):
    ...
    db_items["carrier_grp_ids"] = _execute(
        db_string,
        "select distinct clm_grp_id from carrier_claims where bene_id < -19990000000001 and bene_id != '-88888888888888' order by clm_grp_id asc",
    )
    db_items["inpatient_grp_ids"] = _execute(
        db_string,
        "select distinct clm_grp_id from inpatient_claims where bene_id < -19990000000001 and bene_id != '-88888888888888' order by clm_grp_id asc",
    )
    db_items["outpatient_grp_ids"] = _execute(
        db_string,
        "select distinct clm_grp_id from outpatient_claims where bene_id < -19990000000001 and bene_id != '-88888888888888' order by clm_grp_id asc",
    )
    db_items["pde_grp_ids"] = _execute(
        db_string,
        "select distinct clm_grp_id from partd_events where bene_id < -19990000000001 and bene_id != '-88888888888888' order by clm_grp_id asc",
    )
...
def replace_grp_ids(table_ids, mapped_lists, db_string):
    ...
    process = Process(
        target=replace_ids,
        args=(
            table_ids["carrier_grp_ids"],
            mapped_lists["carrier_grp_ids"],
            "carrier_claims",
            db_string,
        ),
    )
    ...
    process2 = Process(
        target=replace_ids,
        args=(
            table_ids["inpatient_grp_ids"],
            mapped_lists["inpatient_grp_ids"],
            "inpatient_claims",
            db_string,
        ),
    )
    ...
    process3 = Process(
        target=replace_ids,
        args=(
            table_ids["outpatient_grp_ids"],
            mapped_lists["outpatient_grp_ids"],
            "outpatient_claims",
            db_string,
        ),
    )
    ...
    process4 = Process(
        target=split_pde,
        args=(
            table_ids["pde_grp_ids"],
            mapped_lists["pde_grp_ids"],
            "partd_events",
            db_string,
        ),
    )
...
def split_pde(ids_to_replace, replacement_ids, table_name, conn):
    pde_threads = []
    batch_size = 10000
    ids_to_replace_split_list = [
        ids_to_replace[i * batch_size : (i + 1) * batch_size]
        for i in range((len(ids_to_replace) + batch_size - 1) // batch_size)
    ]
