...
import time
=======
from asyncclick import MissingParameter
>>>>>>> Stashed changes
...
def download_synthea_script(target_dir):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "404":
            print(f"The S3 object does not exist: {code_script_file}")
        raise e
    except Exception as e:
...
def download_end_state_props_file(target_dir):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "404":
            print(f"The S3 object does not exist: {end_state_props_file}")
        raise e
    except Exception as e:
...
def upload_end_state_props_file(synthea_output_dir, bfd_dir):
    ...
    try:
        ...
        s3_client.upload_file(
            file_name, mitre_synthea_bucket, "end_state/end_state.properties_orig"
        )
    except ClientError as e:
        if e.response["Error"]["Code"] == "404":
            print(f"The S3 object does not exist: {file_name}")
        raise e
    except Exception as e:
...
def upload_manifest_file(synthea_output_dir, s3_bucket, s3_folder):
    ...
    if os.path.exists(local_fn):
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                print(f"The S3 object does not exist: {remote_fn}")
            raise e
        except Exception as e:
            raise Exception("Unexpected error in upload_manifest_file: " + e.__str__())
    else:
        raise FileNotFoundError(f"Failed to find file: {local_fn}")
...
def upload_synthea_results(synthea_output_dir, s3_bucket):
    ...
    if len(manifest_ts) < 1:
        raise ExtractError("Failed to extract timestamp from manifest")
    if len(s3_bucket) < 1:
        raise MissingParameter("Failed to provide BFD S3 bucket for ETL files")
...
def upload_synthea_result_folder(folder_dir, s3_bucket):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "404":
            print(f"The S3 object does not exist: {destination_fn}")
        raise e
    except Exception as e:
        raise Exception("Unexpected error in upload_manifest_file: " + e.__str__())
