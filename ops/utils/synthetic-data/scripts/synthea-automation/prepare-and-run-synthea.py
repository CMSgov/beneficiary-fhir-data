...
def validate_and_run(args):
    ...
    found_all_paths = validate_file_paths(
        synthea_folder_filepath, synthea_prop_filepath, synthea_output_filepath, end_state_file_path
    )
    ...
    new_end_state_properties_file = read_file_lines(
        synthea_output_filepath + "bfd/end_state.properties"
    )
    update_manifest(
        synthea_output_filepath, end_state_properties_file, new_end_state_properties_file
    )
...
def run_synthea(synthea_folder_filepath, benes_to_generate, future_months):
    ...
    """
    logfile_path = (
        f"{synthea_folder_filepath}synthea-" + time.strftime("%Y_%m_%d-%I_%M_%S_%p") + ".log"
    )
    synthea_failed = False
    if future_months > 0:
        print(
            f"Running synthea ({synthea_folder_filepath}national_bfd.sh) with {benes_to_generate} benes and {future_months} future months..."
        )
        process_cmd = shlex.split(
            f"{synthea_folder_filepath}national_bfd.sh {benes_to_generate} {future_months}"
        )
    else:
        print(
            f"Running synthea ({synthea_folder_filepath}national_bfd.sh) with {benes_to_generate} benes..."
        )
        process_cmd = shlex.split(f"{synthea_folder_filepath}national_bfd.sh {benes_to_generate} 0")
    with (
        subprocess.Popen(
            process_cmd, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1
        ) as p,
...
def validate_file_paths(
    ...
    if not os.path.exists(synthea_folder_filepath):
        print(
            f"(Validation Failure) Synthea folder filepath could not be found at {synthea_folder_filepath}"
        )
        validation_passed = False
    else:
        if not os.path.exists(synthea_prop_filepath):
            print(
                f"(Validation Failure) Synthea properties file could not be found at {synthea_prop_filepath}"
            )
            validation_passed = False
        elif not os.access(synthea_prop_filepath, os.W_OK):
            print(
                f"(Validation Failure) Synthea properties file is not writable (found at {synthea_prop_filepath})"
            )
            validation_passed = False
        if not os.path.exists(synthea_output_filepath):
            print(
                f"(Validation Warning) Output directory ({synthea_output_filepath}) could not be found, creating it..."
            )
            os.mkdir(synthea_output_filepath)
        if not os.access(synthea_output_filepath, os.W_OK):
            print(
                f"(Validation Failure) Synthea output directory is not writable (found at {synthea_output_filepath})"
            )
            validation_passed = False

    if not os.path.exists(end_state_file_path):
        print(
            f"(Validation Failure) End state properties file could not be found at {end_state_file_path}"
        )
        validation_passed = False
    ...
    if os.path.exists(synthea_folder_filepath):
        ...
        for filename in export_filenames:
            ...
            if not os.path.exists(export_file_loc):
                print(
                    f"(Validation Failure) Expected export file could not be found: {export_file_loc}"
                )
                validation_passed = False
        ...
        if not os.path.exists(national_file_loc):
            print(
                f"(Validation Failure) Expected national runfile ({national_file_loc}) could not be found"
            )
            validation_passed = False
        elif not os.access(national_file_loc, os.X_OK):
            print(
                f"(Validation Failure) Synthea run file is not executable (found at {national_file_loc})"
            )
            validation_passed = False
...
def clean_synthea_output(synthea_folder_filepath):
    ...
    if numFiles > 0:
        ...
        os.mkdir(output_dir)
        print(
            "(Validation Warning) Synthea output had files, renamed old output directory and created fresh synthea output folder"
        )
    else:
