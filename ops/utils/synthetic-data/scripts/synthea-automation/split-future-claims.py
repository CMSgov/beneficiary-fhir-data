        ...
        for file_path in files:
            if os.path.isfile(synthea_output_filepath + file_path):
                ...
                future = executor.submit(
                    split_future_from_file, file_path, synthea_output_filepath, today
                )
...
def create_week_folders_and_manifests(file_data_tuples, synthea_output_filepath, today):
    ...
    for file_data_tuple in file_data_tuples:
        ...
        for week_key in file_dict.keys():
            if week_key not in weeks_files:
    ...
    for week in weeks_files:
        week_folder_path = synthea_output_filepath + week
        if not week == today.strftime(manifest_date_format) and not os.path.exists(
            week_folder_path
        ):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i in range(num_threads):
            ...
            if line_end > num_lines:
                line_end = num_lines
            future = executor.submit(
                split_future_lines, full_path, header, lines[line_start:line_end], today
            )
            threads.append(future)
...
def validate_final_lines(file_name, original_count, final_dict):
    ...
    if not original_count == line_count_total:
        print(
            f"(Validation Failure) File {file_name} split into {line_count_total} lines, expected {original_count}"
        )
    else:
        print(
            f"(Validation Success) File {file_name} correctly split into {line_count_total} lines over {len(final_dict.keys())} weeks"
        )
...
def split_future_lines(file_path, header, lines, today):
    ...
    for line in lines:
        ...
        if parsed_date > today:
            ...
            if target_weds.strftime(manifest_date_format) not in week_data:
            ...
            if today.strftime(manifest_date_format) not in week_data:
