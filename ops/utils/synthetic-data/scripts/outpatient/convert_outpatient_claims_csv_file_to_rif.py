        ...
        if row_count == 1:
            for key in row:
                if key not in header_enum_order_list:
                    raise SystemExit(
                        "ERROR: Input file header key is not"
            ...
            if col_name in row:
                ...
                if "_DT" in col_name:
                    ...
                    if dt != "":
                        ...
                        new_dt = dt[0:2] + "-" + dt[2:3] + dt[3:5].lower() + "-" + dt[5:9]

                        # Validate month is OK:
                        if new_dt[3:6] not in valid_month_list:
                            raise SystemExit(
                                "ERROR: Month value in date"
                                " field is not valid: " + col_name + "=" + row[col_name]
                            )
