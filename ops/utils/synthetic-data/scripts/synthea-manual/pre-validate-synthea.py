...
def validate(args):
    ...
    if "test" in envs:
        ...
        test_validation_result = check_ranges(
            end_state_properties_file, generated_benes, test_db_string
        )
        num_run = num_run + 1
    if "prd-sbx" in envs:
        print("Running validations for prod-sbx...")
        prod_sbx_validation_result = check_ranges(
            end_state_properties_file, generated_benes, prod_sbx_db_string
        )
        num_run = num_run + 1
    if "prod" in envs:
        ...
        prod_validation_result = check_ranges(
            end_state_properties_file, generated_benes, prod_string
        )
        num_run = num_run + 1
...
def check_ranges(properties_file, number_of_benes_to_generate, db_string):
    ...
    overall_validation_result = field_has_room_in_table(
        clm_id_start, "carrier_claims", "clm_id", result, overall_validation_result
    )
    ...
    result = _execute_single_count_query(db_string, query)
    overall_validation_result = field_has_room_in_table(
        bene_id_start, "beneficiaries", "bene_id", result, overall_validation_result
    )
    ...
    result = _execute_single_count_query(db_string, query)
    overall_validation_result = field_has_room_in_table(
        pde_id_start, "partd_events", "pde_id", result, overall_validation_result
    )

    carr_clm_ctrl_num_start = get_props_value(
        properties_file, "exporter.bfd.carr_clm_cntl_num_start"
    )
    query = f"select count(*) from carrier_claims where carr_clm_cntl_num::bigint <= {carr_clm_ctrl_num_start}"
    result = _execute_single_count_query(db_string, query)
    overall_validation_result = field_has_room_in_table(
        carr_clm_ctrl_num_start,
        "carrier_claims",
        "carr_clm_cntl_num",
        result,
    ...
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "outpatient_claims", "fi_doc_clm_cntl_num", fi_num_start, db_string
    )
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "inpatient_claims", "fi_doc_clm_cntl_num", fi_num_start, db_string
    )
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "hha_claims", "fi_doc_clm_cntl_num", fi_num_start, db_string
    )
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "snf_claims", "fi_doc_clm_cntl_num", fi_num_start, db_string
    )
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "hospice_claims", "fi_doc_clm_cntl_num", fi_num_start, db_string
    )
    ...
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "beneficiaries", "hicn_unhashed", hicn_start, db_string
    )

    mbi_start = get_props_value(properties_file, "exporter.bfd.mbi_start")
    overall_validation_result = overall_validation_result and check_single_doesnt_exist(
        "beneficiaries", "mbi_num", mbi_start, db_string
    )
    ...
    if result > 1:
        print(
            f"(Validation Warning) {result} MBI(s) resolve to multiple bene_ids; a given mbi should only resolve to one bene_id (this needs cleanup to avoid errors using that data)"
        )
    else:
...
def get_props_value(list, starts_with):
    ...
    """
    return next((x.split("=", 1)[1] for x in list if x.startswith(starts_with)), None)
...
def field_has_room_in_table(starting_value, table, field, query_result, overall_validation_result):
    ...
    if query_result > 0:
        print(
            f"(Validation Failure) {table} : {field} has conflict with data to load, {query_result} rows beyond starting value {starting_value}"
        )
        return False
    print(
        f"(Validation Success) {table} : {field} has clearance from starting value {starting_value}"
    )
    return overall_validation_result
