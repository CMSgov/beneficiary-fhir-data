...
def validate_synthea_load(args):
    ...
    if Path(synthea_output_folder + "carrier.csv").is_file():
        table_ids["carrier"] = get_bene_id_from_last_file_line(
            synthea_output_folder + "carrier.csv"
        )
    ...
    if Path(synthea_output_folder + "hospice.csv").is_file():
        table_ids["hospice"] = get_bene_id_from_last_file_line(
            synthea_output_folder + "hospice.csv"
        )
    if Path(synthea_output_folder + "inpatient.csv").is_file():
        table_ids["inpatient"] = get_bene_id_from_last_file_line(
            synthea_output_folder + "inpatient.csv"
        )
    if Path(synthea_output_folder + "outpatient.csv").is_file():
        table_ids["outpatient"] = get_bene_id_from_last_file_line(
            synthea_output_folder + "outpatient.csv"
        )
    ...
    if "test" in envs:
        ...
        test_validation_result = check_data_loaded(
            bene_id_start, bene_id_end, expected_benes, table_ids, test_db_string
        )
        num_run = num_run + 1
    if "prd-sbx" in envs:
        print("Running validations for prod-sbx...")
        prod_sbx_validation_result = check_data_loaded(
            bene_id_start, bene_id_end, expected_benes, table_ids, prod_sbx_db_string
        )
        num_run = num_run + 1
    if "prod" in envs:
        print("Running validations for prod...")
        prod_validation_result = check_data_loaded(
            bene_id_start, bene_id_end, expected_benes, table_ids, prod_string
        )
        num_run = num_run + 1
...
def check_data_loaded(bene_id_start, bene_id_end, expected_benes, table_ids, db_string):
    ...
    if not result == expected_benes:
        print(
            f"(Validation Failure) expected {expected_benes} but only found {result} between bene_ids {bene_id_start} and {bene_id_end}"
        )
    ...
    if "carrier" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["carrier"], "carrier_claims", db_string)
            and total_result
        )
    if "dme" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["dme"], "dme_claims", db_string) and total_result
        )
    if "hha" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["hha"], "hha_claims", db_string) and total_result
        )
    if "inpatient" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["inpatient"], "inpatient_claims", db_string)
            and total_result
        )
    if "hospice" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["hospice"], "hospice_claims", db_string)
            and total_result
        )
    if "outpatient" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["outpatient"], "outpatient_claims", db_string)
            and total_result
        )
    if "pde" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["pde"], "partd_events", db_string) and total_result
        )
    if "snf" in table_ids:
        total_result = (
            check_table_for_bene_id(table_ids["snf"], "snf_claims", db_string) and total_result
        )
