...
-- 1. LOCATE THE BACKUP DUMP FILES ON S3
  ...
  --1a.  Check if backup files are on S3 by running /opt/backups/db_list.bash script. 
         e.g. for backup date 2019-05-07, run the following:
 
	$ /opt/backups/db_list.bash | grep 2019-05-07/
							   PRE 2019-05-07/

  --1b. List the backup files for the backup date (e.g. 2019-05-07)
  ...
  --1c. Restore Backup From S3 To Local Disk for e.g 2019-05-07.
        This 
...
-- 2. CREATE/REVIEW  RESTORE SCRIPT pg_restore_tables.sh
	...
	-- 2a. review the pg_restore_tables.sh script to ensure all tables in the dump files to be restored are included.
	       --IMPORTANT : Aall other tables in the database reference the Beneficiaries table, therefore this table
	...
	-- 2b. Review the contents of each table restore script. 
           e.g. for Beneficiaries and CarrierClaims tables
...
-- 3. DROP EXISTING TABLES
	...
	--3c. Check for existence of dropped tables. Should return no rows.
...
-- 6. SET AUTOVACUUM ON AFTER RESTORE COMPLETES SUCCESSFULLY
