...
if ansible-playbook "$TEST_PLAY" --inventory=inventory.docker.yaml --extra-vars "bfd_version=${BFD_VERSION} ${EXTRA_VARS}" | tee /dev/stderr | grep -q "${CONTAINER_NAME}.*changed=0.*failed=0"; then
