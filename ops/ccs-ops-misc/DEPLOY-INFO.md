...
- Deployment stages use locks to prevent multiple deployments from occurring at once to the same environment.
...
In circumstances where the old deployment autoscaling group is online and no outage has occurred, the best option is to roll forward and attempt another deployment with any associated fixes.
