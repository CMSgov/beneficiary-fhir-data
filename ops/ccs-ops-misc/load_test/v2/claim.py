...
class BFDUser(HttpUser):
    ...
    def get_claim_with_service_date(self):
        self.base_query(f'/v2/fhir/Claim?mbi={self.hashed_mbi()}&service-date=gt2020-01-05', 'claimServiceDate')

    @tag('claim', 'last-updated')
    @task
    def get_claim_with_last_updated(self):
        self.base_query(f'/v2/fhir/Claim?mbi={self.hashed_mbi()}&_lastUpdated=gt2020-05-05', 'claimLastUpdated')

    @tag('claim', 'service-date', 'last-updated')
    @task
    def get_claim_with_service_date_and_last_updated(self):
        self.base_query(f'/v2/fhir/Claim?mbi={self.hashed_mbi()}&_lastUpdated=gt2020-05-05&service-date=gt2020-01-05', 'claimServiceDateLastUpdated')
