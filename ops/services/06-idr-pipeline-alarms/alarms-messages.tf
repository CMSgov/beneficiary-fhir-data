...
resource "aws_cloudwatch_metric_alarm" "errors" {
  ...
  statistic           = "Sum"
  threshold           = 0
  alarm_description   = "IDR Pipeline errors detected in ${local.env}"
...
resource "aws_cloudwatch_metric_alarm" "log_availability_24hr" {
  alarm_name          = "${local.alarms_prefix}-log-availability-24hr"
  ...
  alarm_description = join("", [
    "IDR Pipeline logs have not been submitted to CloudWatch in 24 hours, pipeline may not have run ",
