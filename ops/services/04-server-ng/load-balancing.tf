locals {
  ...
  lb_internal_vpc_peering_cidrs = [for connection in module.terraservice.all_connections : connection.foreign_cidr if connection.env == local.env]
...
resource "aws_route53_record" "this" {
  ...
  name    = "${local.env}.fhirv3.${one(data.aws_route53_zone.root[*].name)}"
  type    = "A"
