        ...
        running_idr_tasks = (
            [
                ...
                for task in ecs_client.describe_tasks(
                    cluster=settings.ecs_cluster_arn, tasks=tasks
                )["tasks"]
                if "group" in task
        ...
        if len(running_idr_tasks) > 0:
            if invoke_model.reschedule:
                ...
                try:
                    ...
                    logger.exception(
                        "An error occurred when trying to delete old schedules; continuing"
                    )
                ...
                scheduler_client.create_schedule(
                    GroupName=settings.idr_task_schedules_group,
                    Name=schedule_name,
                    ScheduleExpression=f"at({reschedule_for.isoformat(timespec='seconds')})",
                    ScheduleExpressionTimezone="UTC",
                    Target={
                        "RoleArn": settings.lambda_role_arn,
