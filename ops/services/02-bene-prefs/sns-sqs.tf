...
data "aws_iam_policy_document" "sns_allow_s3_publish" {
  ...
  statement {
    ...
    condition {
      ...
      values   = [module.eft_bucket[each.key].bucket.arn]
    }
  }
}
...
data "aws_iam_policy_document" "sqs_allow_sns_send" {
  ...
  statement {
    ...
    condition {
      test     = "ArnEquals"
      variable = "aws:SourceArn"
      values   = [aws_sns_topic.partner_bucket_events[each.key].arn]
    }
  }
}
...
resource "aws_sns_topic_subscription" "partner_bucket_events_to_sqs" {
  ...
  protocol  = "sqs"
  endpoint  = aws_sqs_queue.partner_bucket_events[each.key].arn
}
