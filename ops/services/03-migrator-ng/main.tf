...
module "terraservice" {
  ...
  relative_module_root = "ops/services/03-migrator-ng"
...
locals {
  ...
  migrator_repository_name = coalesce(var.migrator_repository_override, "bfd-platform-migrator-ng")
  ...
  migrator_task_ssm = {
    for k, v in {
      FLYWAY_USER     = "/bfd/${local.env}/${local.service}/sensitive/db/username"
      FLYWAY_PASSWORD = "/bfd/${local.env}/${local.service}/sensitive/db/password"
    ...
    [
      {
        ...
        cpu       = 0
        secrets = [
          for k, v in local.migrator_task_ssm :
          {
            name      = k
            valueFrom = v
          }
...
resource "null_resource" "start_migrator" {
  ...
  provisioner "local-exec" {
    environment = {
      ...
      LOG_GROUP_NAME = module.messages.name
    }
