...
resource "aws_vpc_security_group_ingress_rule" "allow_adot" {
  security_group_id            = data.aws_security_group.adot.id
  referenced_security_group_id = aws_security_group.server.id
  from_port                    = 4317
  to_port                      = 4317
  ip_protocol                  = "TCP"
  description                  = "Grants server service to the adot service"
}
