...
resource "aws_glue_crawler" "glue_crawler_api_requests" {
  depends_on = [module.glue_table_api_requests]
  ...
  role = data.aws_iam_role.iam_role_glue.arn

  catalog_target {
    database_name = module.database.name
    tables = [
      module.glue_table_api_requests.name,
    ]
  }
...
module "lambda_trigger_crawler" {
  ...
  lambda_name = "${local.full_name}-trigger-crawler"

  kms_key_arn   = local.env_key_arn
  crawler_name  = aws_glue_crawler.glue_crawler_api_requests.name
  crawler_arn   = aws_glue_crawler.glue_crawler_api_requests.arn
  database_name = module.database.name
