...
public final class BenchmarkCleanup {
	 ...
	 *            (not used)
	 * @throws Exception
	 *             Any {@link Exception}s encountered will cause this app to
	 *             terminate early.
	 */
	public static void main(String[] args) throws Exception {
		...
		Set<Integer> iterationIndices = Files.list(benchmarksWorkDir).filter(p -> Files.isDirectory(p))
				.map(p -> p.getFileName().toString()).map(i -> Integer.parseInt(i)).collect(Collectors.toSet());

		for (int iterationIndex : iterationIndices) {
			int teardownExitCode = BenchmarkUtilities.runBenchmarkTeardown(iterationIndex,
					benchmarksWorkDir.resolve("" + iterationIndex), ec2KeyFile);
			System.out.printf("Iteration '%d': teardown returned '%d'.\n", iterationIndex, teardownExitCode);

			try {
				Bucket iterationDataBucket = new Bucket(
				...
				System.out.printf("Iteration '%d': unable to delete S3 bucket ('%s').\n", iterationIndex,
						e.getMessage());
			}
		}
	}
