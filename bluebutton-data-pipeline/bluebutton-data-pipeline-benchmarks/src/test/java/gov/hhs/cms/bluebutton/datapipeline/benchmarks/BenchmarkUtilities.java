...
final class BenchmarkUtilities {
	...
	/**
	 * @return the {@link Path} specified by the value of the
	 *         {@link BenchmarkUtilities#SYS_PROP_EC2_KEY_FILE} Java system
	 *         property
	 */
	static Path findEc2KeyFile() {
		String ec2KeyFile = System.getProperty(BenchmarkUtilities.SYS_PROP_EC2_KEY_FILE, null);
		if (ec2KeyFile == null)
			throw new IllegalArgumentException(String.format("The '%s' Java system property must be specified.",
					BenchmarkUtilities.SYS_PROP_EC2_KEY_FILE));
		Path ec2KeyFilePath = Paths.get(ec2KeyFile);
		if (!Files.isReadable(ec2KeyFilePath))
			throw new IllegalArgumentException(
					String.format("The '%s' Java system property must specify a valid key file.",
							BenchmarkUtilities.SYS_PROP_EC2_KEY_FILE));
	...
	/**
	 * @return the {@link Path} to this project's
	 *         <code>target/benchmark-iterations</code> directory
	 */
	 ...
	 *            the index/ID of the benchmark iteration to compute the bucket
	 *            name for
	 * @return the name of the S3 bucket that the specified benchmark iteration
	...
	/**
	 * Runs the <code>teardown.sh</code> script for the specified benchmark
	 * iteration. This script will collect any relevant logs, then terminate the
	 * EC2 instances, etc. for the specified benchmark iteration.
	 * 
	 * @param iterationIndex
	 *            the index/ID of the benchmark iteration to clean up
	 * @param benchmarkIterationDir
	 *            the {@link Path} of the directory that the benchmark iteration
	...
	static int runBenchmarkTeardown(int iterationIndex, Path benchmarkIterationDir, Path ec2KeyFile) {
		...
		int teardownExitCode = BenchmarkUtilities.runProcessAndLogOutput(teardownProcessBuilder, teardownLog);
