...
* Determine whether or not miniStack is running.
    * If it is all settings that can be will be copied into miniStack SSM and the apps will run in SSM mode.
...
See the section on MiniStack below for instructions on how to find that IP address.
...
MiniStack can be used to run the apps using SSM configuration instead of environment variables.
...
The scripts will auto-detect when MiniStack is running by checking port 4566 (the MiniStack port)
for a listener. If one is found all configuration settings will be stored in MiniStack
...
Because of the complexities of docker networking the app will not be able to find a MiniStack
...
In this example the MiniStack container is named `ministack_main` and its bridge network IP address is `172.17.0.3`.
To allow a docker container to communicate with MiniStack you would need to add the command line option `-l 172.17.0.3`
...
-l ministack_ip       MiniStack IP address (for bridge networking).
