...
| `--stats-config` | `LOCUST_STATS_CONFIG` | `stats-config` | No | `None` | Argument specifying that aggregated performance statistics should be collected and stored to some location. This can either be to a local file or to an S3 bucket. This argument must be specified as a list of key-value pairs seperated by semi-colons: `--stats-config="store=<file/s3>;env=<TEST/PROD>;store_tag=;path=;bucket=;database=;table=;compare=<previous/average>;comp_tag=;"`. See [`--stats_config` Reference](#stats-config-ref) below for more information on this argument
