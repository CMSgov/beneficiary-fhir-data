...
public interface RdaMessageSourceFactory extends AutoCloseable {
  ...
  class Config implements Serializable {
    ...
    public RdaMessageSourceFactory createMessageSourceFactory() throws Exception {
      RdaMessageSourceFactory answer;
      if (fissSourceFactory != null || mcsSourceFactory != null) {
        answer = createBasicMessageSourceFactory();
      } else if (fissClaimJson != null || mcsClaimJson != null) {
        answer = createJsonMessageSourceFactory();
      } else if (s3Bucket != null) {
        answer = createS3MessageSourceFactory();
      } else {
        answer = createRandomMessageSourceFactory();
      }
      if (throwExceptionAfterCount > 0) {
        answer = new RdaExceptionMessageSourceFactory(answer, throwExceptionAfterCount);
      }
      return answer;
    }
    ...
    public static class ConfigBuilder {
      /**
       * Calls {@link Config#fissClaimJson} with a {@link ByteSource} constructed by combining the
      ...
      public ConfigBuilder fissClaimJsonList(List<String> jsonChanges) {
        ...
        var combinedBytes = combinedString.getBytes(StandardCharsets.UTF_8);
        return fissClaimJson(ByteSource.wrap(combinedBytes));
      }

      /**
       * Calls {@link Config#mcsClaimJson} with a {@link ByteSource} constructed by combining the
       * provided JSON strings.
       *
      ...
      public ConfigBuilder mcsClaimJsonList(List<String> jsonChanges) {
        var combinedString = String.join("\n", jsonChanges);
        var combinedBytes = combinedString.getBytes(StandardCharsets.UTF_8);
        return mcsClaimJson(ByteSource.wrap(combinedBytes));
      }

      /**
       * Optionally add a {@link File} as a source of FISS claim data. The argument can be null so
       * that this can be called when a file may or may not be available.
       *
      ...
      public ConfigBuilder fissClaimJsonFile(@Nullable File ndjsonFile) {
        if (ndjsonFile != null) {
          fissClaimJson(Files.asByteSource(ndjsonFile));
        }
        return this;
      }

      /**
       * Optionally add a {@link File} as a source of MCS claim data. The argument can be null so
       * that this can be called when a file may or may not be available.
       *
       * @param ndjsonFile null or a valid {@link File} containing ndjson data
       * @return this builder
      ...
      public ConfigBuilder mcsClaimJsonFile(@Nullable File ndjsonFile) {
        if (ndjsonFile != null) {
          mcsClaimJson(Files.asByteSource(ndjsonFile));
        }
        return this;
      }
    }
