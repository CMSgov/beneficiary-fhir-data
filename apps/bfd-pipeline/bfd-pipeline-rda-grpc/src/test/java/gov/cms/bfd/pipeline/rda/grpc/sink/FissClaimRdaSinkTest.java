...
public class FissClaimRdaSinkTest {
  ...
  public void persistSuccessful() throws Exception {
    ...
    for (RdaChange<PreAdjFissClaim> change : batch) {
      verify(entityManager).persist(wrapClaim(change.getClaim()));
    }
  ...
  public void mergeSuccessful() throws Exception {
    ...
    doThrow(new EntityExistsException())
        .when(entityManager)
        .persist(wrapClaim(batch.get(1).getClaim()));

    final int count = sink.writeBatch(batch);
    assertEquals(3, count);

    verify(entityManager).persist(wrapClaim(batch.get(0).getClaim()));
    verify(entityManager).persist(wrapClaim(batch.get(1).getClaim()));
    verify(entityManager, times(0))
        .persist(wrapClaim(batch.get(2).getClaim())); // not called once a persist fails
    for (RdaChange<PreAdjFissClaim> change : batch) {
      verify(entityManager).merge(wrapClaim(change.getClaim()));
    }
  ...
  public void persistAndMergeFail() {
    ...
    doThrow(new EntityExistsException())
        .when(entityManager)
        .persist(wrapClaim(batch.get(0).getClaim()));
    doThrow(new EntityNotFoundException())
        .when(entityManager)
        .merge(wrapClaim(batch.get(1).getClaim()));
    ...
    verify(entityManager).merge(wrapClaim(batch.get(0).getClaim()));
    verify(entityManager).merge(wrapClaim(batch.get(1).getClaim()));
    verify(entityManager, times(0))
        .persist(wrapClaim(batch.get(2).getClaim())); // not called once a merge fails
