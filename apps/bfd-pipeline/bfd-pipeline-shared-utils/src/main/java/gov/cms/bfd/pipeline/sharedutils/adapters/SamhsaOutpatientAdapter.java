...
public class SamhsaOutpatientAdapter
   ...
   */
  public SamhsaOutpatientAdapter(OutpatientClaim claim) {
  ...
  Map<Supplier<Optional<String>>, String> getClaimMethods() {
    return Map.ofEntries(
        ...
        entry(claim::getDiagnosisAdmission1Code, RSN_VISIT_CD_1),
        entry(claim::getDiagnosisAdmission2Code, RSN_VISIT_CD_2),
        entry(claim::getDiagnosisAdmission3Code, RSN_VISIT_CD_3),
  ...
  public boolean checkAndProcessClaim(EntityManager entityManager)
    ...
    if (entries.isPresent()) {
      List<OutpatientTag> tags = new ArrayList<>();
      tags.add(
