...
def _idr_dates_from_meta_sk() -> str:
    """
    Generates SQL expressions to derive insert and update timestamps
    from META keys.
    ...
    date_from_meta_lst_updt_sk = f"""(meta_lst_updt_sk / {date_sequence_divisor}) + {date_base_offset}"""
    ...
    return f"""
            ...
            CASE
                WHEN meta_sk = 501 THEN
                    TO_TIMESTAMP(
                            TO_DATE(
                                    TRUNC({date_from_meta_lst_updt_sk})::text,
                                    'YYYYMMDD'
                            )::text || ' 00:00:00',
