...
def get_referenced_maps(compiled_map_path):
    try:
        with open(compiled_map_path, 'r') as f:
...
def get_structure_definitions():
    try:
        ...
        for file in source_dir.glob("*.json"):
            structure_defs.append(f"StructureDefinitions/Source/{file.name}")
...
def get_sushi_resources():
    try:
        ...
        for file in sushi_dir.glob("*.json"):
            sushi_resources.append(f"sushi/fsh-generated/resources/{file.name}")
...
def run_conformance_test(input_file, output_file):
    try:
        ...
        test_cmd = f"java -jar validator_cli.jar {input_file} -output {output_file} -version 4.0.1 {structure_defs} -ig hl7.fhir.us.carin-bb#2.1.0 {sushi_resources}"
...
def main():
    ...
    parser.add_argument('--map', '-m', type=str, help='Path to the structure map file', required=True)
    parser.add_argument('--input', '-i', type=str, help='Path to the input JSON file', required=True)
    parser.add_argument('--output', '-o', type=str, help='Path to the output JSON file', required=True)
    parser.add_argument('--resource', '-r', type=str, help='Resource URL for the structure map', required=True)
    parser.add_argument('--test', '-t', action='store_true', help='Run conformance testing after transformation')
    args = parser.parse_args()

    script_dir = Path(__file__).parent.absolute()
    
    #When we convert the existing StructureDefinitions to .fsh we won't need this, it'll just generate them with SUSHI.
    structure_defs = get_structure_definitions()
    ...
    compile_cmd = f"java -jar validator_cli.jar -ig {args.map} -compile {args.resource} -version 4.0.1 -output {compiled_map_path}"
    ...
    execute_cmd = f"java -jar validator_cli.jar {args.input} -output {args.output} -transform {args.resource} -version 4.0.1 -ig {compiled_map_path} {structure_defs} -ig hl7.fhir.us.carin-bb#2.1.0 {map_imports} {sushi_resources}"
