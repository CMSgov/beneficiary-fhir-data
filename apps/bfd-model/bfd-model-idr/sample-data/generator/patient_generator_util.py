...
class PatientGeneratorUtil:
    ...
    def handle_mbis(self, patient, num_mbis, custom_first_mbi=None):
        for mbi_idx in range(0, num_mbis):
            ...
            if (mbi_idx == 0):
                efctv_dt = self.fake.date_between_dates(datetime.date(year=2017, month=5, day=20), 
                ...
                efctv_dt = self.fake.date_between_dates(datetime.date(year=2021, month=1, day=2),
    ...
    def save_output_files(self):
        ...
        df = df[['BENE_SK','BENE_XREF_EFCTV_SK','BENE_MBI_ID','BENE_LAST_NAME','BENE_1ST_NAME','BENE_MIDL_NAME','BENE_DOB','BENE_DEATH_DT','BENE_VRFY_DEATH_DAY_SW','BENE_SEX_CD','BENE_RACE_CD','BENE_LINE_1_ADR','BENE_LINE_2_ADR','BENE_LINE_3_ADR','BENE_LINE_4_ADR','BENE_LINE_5_ADR','BENE_LINE_6_ADR','GEO_ZIP_PLC_NAME','GEO_ZIP5_CD','GEO_USPS_STATE_CD', 'CNTCT_LANG_CD','IDR_TRANS_EFCTV_TS','IDR_UPDT_TS','IDR_TRANS_OBSLT_TS']]
        df.to_csv('SYNTHETIC_BENE.csv', index=False)

        df = pd.json_normalize(self.bene_hstry_table)
        df = df[['BENE_SK','BENE_XREF_EFCTV_SK','BENE_MBI_ID','BENE_LAST_NAME','BENE_1ST_NAME','BENE_MIDL_NAME','BENE_DOB','BENE_DEATH_DT','BENE_VRFY_DEATH_DAY_SW','BENE_SEX_CD','BENE_RACE_CD','BENE_LINE_1_ADR','BENE_LINE_2_ADR','BENE_LINE_3_ADR','BENE_LINE_4_ADR','BENE_LINE_5_ADR','BENE_LINE_6_ADR','GEO_ZIP_PLC_NAME','GEO_ZIP5_CD','GEO_USPS_STATE_CD', 'CNTCT_LANG_CD','IDR_TRANS_EFCTV_TS','IDR_UPDT_TS','IDR_TRANS_OBSLT_TS']]
        df.to_csv('SYNTHETIC_BENE_HSTRY.csv', index=False)

        arr = [{'BENE_MBI_ID': mbi, **self.mbi_table[mbi]} for mbi in self.mbi_table]
        df = pd.json_normalize(arr)
        df = df[['BENE_MBI_ID','BENE_MBI_EFCTV_DT','BENE_MBI_OBSLT_DT','IDR_TRANS_EFCTV_TS','IDR_UPDT_TS','IDR_TRANS_OBSLT_TS']]
        df.to_csv('SYNTHETIC_BENE_MBI_ID.csv', index=False) 