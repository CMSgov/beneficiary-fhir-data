/*
  This migration adds a set of database admin functions to help us manage role-based access and ownership, while keeping versioned
  migrations DRY. It duplicates the ./createSchema.sql callback to ensure the db admin functions are added to *existing* databases
  (the createSchema.sql callback is only run once on new database installations). Note the createSchema.sql contents may drift from
  this migration over time.
