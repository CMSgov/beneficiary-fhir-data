create schema if not exists part_adj;

create table part_adj.rda_api_progress (
    claim_type           varchar(20) not null primary key,
    last_sequence_number bigint      not null,
    last_updated         timestamp with time zone
);

create table part_adj.mbi_cache (
...
create unique index mbi_cache_mbi_idx on part_adj.mbi_cache(mbi);

create unique index mbi_cache_hash_idx on part_adj.mbi_cache(hash);

create index mbi_cache_old_hash_idx on part_adj.mbi_cache(old_hash);

create sequence part_adj.rda_api_claim_message_meta_data_meta_data_id_seq as bigint ${logic.sequence-start} 1 ${logic.sequence-increment} 25 no cycle;

create table part_adj.rda_api_claim_message_meta_data (
    ...
    foreign key (mbi_id) references part_adj.mbi_cache(mbi_id)
);


create table part_adj.fiss_claims (
...
create index fiss_claims_last_updated_idx
    on part_adj.fiss_claims(last_updated);

create index fiss_claims_mbi_id_idx
    on part_adj.fiss_claims(mbi_id);

create table part_adj.fiss_audit_trails (
    ...
    foreign key (dcn) references part_adj.fiss_claims(dcn)
);

create table part_adj.fiss_diagnosis_codes (
    dcn          varchar(23) not null,
    priority     smallint    not null,
...
create table part_adj.fiss_proc_codes (
    dcn          varchar(23) not null,
    priority     smallint    not null,
...
create table part_adj.fiss_payers (
    dcn                varchar(23) not null,
    priority           smallint    not null,
...
create table part_adj.mcs_claims (
...
create index mcs_claims_last_updated_idx
    on part_adj.mcs_claims(last_updated);

create index mcs_claims_mbi_id_idx
    on part_adj.mcs_claims(mbi_id);

create table part_adj.mcs_adjustments (
    ...
    foreign key (idr_clm_hd_icn) references part_adj.mcs_claims(idr_clm_hd_icn)
);

create table part_adj.mcs_audits (
    idr_clm_hd_icn   varchar(15) not null,
    priority         smallint    not null,
    last_updated     timestamp with time zone,
...
create table part_adj.mcs_details (
    idr_clm_hd_icn              varchar(15) not null,
    priority                    smallint    not null,
...
create table part_adj.mcs_diagnosis_codes (
    idr_clm_hd_icn    varchar(15) not null,
    priority          smallint    not null,
...
create table part_adj.mcs_locations (
    idr_clm_hd_icn    varchar(15) not null,
    priority          smallint    not null,
    last_updated      timestamp with time zone,
