...
DECLARE
  ...
  v_bene_id		bigint  := 0;
  v_clm_id      bigint  := 0;
  v_tbl_name	varchar(40) := 'hha_claims';
...
BEGIN
	...
	loop
		...
		select into curr
			...
			clm_freq_cd as f_10,
			clm_hha_lupa_ind_cd as f_11,
			clm_hha_rfrl_cd as f_12,
			clm_hha_tot_visit_cnt as f_13,
			clm_mdcr_non_pmt_rsn_cd as f_14,
			clm_pps_ind_cd as f_15,
			clm_srvc_clsfctn_type_cd as f_16,
			...
			nch_clm_type_cd as f_24,
			nch_near_line_rec_ident_cd as f_25,
			nch_prmry_pyr_cd as f_26,
			nch_prmry_pyr_clm_pd_amt as f_27,
			nch_wkly_proc_dt as f_28,
			org_npi_num as f_29,
			prncpal_dgns_cd as f_30,
			prncpal_dgns_vrsn_cd as f_31,
			prvdr_num as f_32,
			prvdr_state_cd as f_33,
			ptnt_dschrg_stus_cd as f_34,
			rev_cntr_tot_chrg_amt as f_35,
			at_physn_npi as f_36,
			at_physn_upin as f_37,
		...
		SELECT INTO orig
			...
			"dateFrom" as f_5,
			"dateThrough" as f_6,
			"paymentAmount" as f_7,
			"careStartDate" as f_8,
			"claimFacilityTypeCode" as f_9,
			"claimFrequencyCode" as f_10,
			"claimLUPACode" as f_11,
			"claimReferralCode" as f_12,
			"totalVisitCount" as f_13,
			"claimNonPaymentReasonCode" as f_14,
			"prospectivePaymentCode" as f_15,
			"claimServiceClassificationTypeCode" as f_16,
			...
			"claimTypeCode" as f_24,
			"nearLineRecordIdCode" as f_25,
			"claimPrimaryPayerCode" as f_26,
			"primaryPayerPaidAmount" as f_27,
			"weeklyProcessDate" as f_28,
			"organizationNpi" as f_29,
			"diagnosisPrincipalCode" as f_30,
			"diagnosisPrincipalCodeVersion" as f_31,
			"providerNumber" as f_32,
			"providerStateCode" as f_33,
			"patientDischargeStatusCode" as f_34,
			"totalChargeAmount" as f_35,
			"attendingPhysicianNpi" as f_36,
			"attendingPhysicianUpin" as f_37,
