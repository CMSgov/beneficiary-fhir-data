...
DECLARE
  ...
  v_bene_id		bigint  := 0;
  v_tbl_name	varchar(40) := 'medicare_beneficiaryid_history';

BEGIN
	v_beneIds := ARRAY(
		SELECT distinct cast("beneficiaryId" as bigint)
		FROM "MedicareBeneficiaryIdHistory" TABLESAMPLE BERNOULLI(50)	-- bernoulli sample using 50% of table rows
		limit MAX_TESTS);
