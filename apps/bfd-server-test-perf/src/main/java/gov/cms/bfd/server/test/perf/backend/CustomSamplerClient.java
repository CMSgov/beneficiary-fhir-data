...
/**
 * {@link CustomSamplerClient} encapsulates the common variables and methods that are shared among
 * all tests in this package. Simply derive from this class and implement {@link
 * CustomSamplerClient#executeTest()} at a minimum to create new test functionality.
 */
public abstract class CustomSamplerClient extends AbstractJavaSamplerClient {
  ...
  /**
   * Abstract method that should be implemented by derived classes to implement test functionality
   * specific to the desired test case.
   */
  ...
  public Arguments getDefaultParameters() {
    ...
    defaultParameters.addArgument(
        PARAM_SERVER, "https://fhir.backend.bluebutton.hhsdevcloud.us/v1/fhir");
    defaultParameters.addArgument(
        KEYSTORE_DIR, "/usr/local/bluebutton-jmeter-service/dev/ssl-stores");
  ...
  public void setupTest(JavaSamplerContext context) {
    ...
    if (context.getParameter(PROXY_HOST).equals("null")) { // no proxy
      this.client =
          FhirClient.create(context.getParameter(PARAM_SERVER), context.getParameter(KEYSTORE_DIR));
    } else { // use proxy
      this.client =
          FhirClient.create(
    ...
    // RifFileType.valueOf(context.getParameter(RIFTYPE).toUpperCase()));
  ...
  /**
   * @see
   *     org.apache.jmeter.protocol.java.sampler.JavaSamplerClient#runTest(org.apache.jmeter.protocol.java.sampler.JavaSamplerContext)
   */
  @Override
  ...
  /**
   * @return the hostname of the system this code is running on, or {@link #HOSTNAME_UNKNOWN} is it
   *     can't be determined
   */
  protected static String getHostname() {
    String hostname = null;
    if (hostname == null) hostname = System.getenv().get("HOSTNAME");
    if (hostname == null) hostname = System.getenv().get("COMPUTERNAME");
    try {
      if (hostname == null) hostname = Inet4Address.getLocalHost().getHostName();
