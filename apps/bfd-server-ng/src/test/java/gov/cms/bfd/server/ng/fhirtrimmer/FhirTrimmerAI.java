...
public class FhirTrimmerAI {
  ...
  /**
   * Evaluates a FHIRPath expression against a resource and removes all matching elements. This
   * operation modifies the provided resource in-place.
   *
  ...
  public void trim(IBaseResource resource, String fhirPathExpression) {
    ...
    List<IBase> targets =
        fhirContext.newFhirPath().evaluate(resource, fhirPathExpression, IBase.class);
    if (targets.isEmpty()) {
  ...
  private void trimElement(IBase element, Set<IBase> targetsToRemove, Set<IBase> visited) {
    ...
    for (Method method : element.getClass().getMethods()) {
      ...
      if (methodName.startsWith("get")
          && !methodName.startsWith("getOrCreate")
        ...
        // Case B: The property is a singular element (e.g., Patient.birthDate,
