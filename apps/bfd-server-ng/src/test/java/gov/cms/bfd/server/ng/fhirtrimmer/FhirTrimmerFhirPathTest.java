...
class FhirTrimmerFhirPathTest {
    ...
    void setUp() {
        ...
        testClaim.addExtension(new Extension("http://example.com/ext/bad", new StringType("Remove Me")));
    }
    ...
    void testTrim_SimpleFieldRemoval() {
        ...
        Map<String, List<String>> profileMap = Map.of(
                "Strict", List.of("Claim.status", "Claim.billablePeriod")
        );
        FhirTrimmerFhirPath trimmer = new FhirTrimmerFhirPath(profileMap, fhirContext);
    ...
    void testTrim_CompleteCollectionRemoval() {
        ...
        // Assert
        assertTrue(trimmedClaim.getExtension().isEmpty(), "All extensions should be completely stripped");
    ...
    void testTrim_ConditionalPathRemovalWithWhereClause() {
        ...
        Map<String, List<String>> profileMap = Map.of(
                "Filtered", List.of("Claim.extension.where(url = 'http://example.com/ext/bad')")
        );
        FhirTrimmerFhirPath trimmer = new FhirTrimmerFhirPath(profileMap, fhirContext);
    ...
    void benchmarkFhirTrimmerFhirPathPerformance() {
        ...
        System.out.println("Starting load test of " + totalIterations + " Claims (FhirTrimmerFhirPath)...");
        ...
        for (int i = 0; i < totalIterations; i++) {
            ...
            watch.suspend();
            assertTrue(returned.getExtension().isEmpty(), "FhirTrimmerFhirPath failed to strip extensions");
            watch.resume();
