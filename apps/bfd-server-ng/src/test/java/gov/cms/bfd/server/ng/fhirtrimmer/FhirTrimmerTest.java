...
class FhirTrimmerTest {
  ...
  private ExplanationOfBenefit buildTemplateEob() {
    ...
    eob.getType()
        .addCoding(
            new Coding(
                "https://bluebutton.cms.gov/fhir/CodeSystem/CLM-TYPE-CD", "60", "Inpatient"));
    ...
    for (int i = 0; i < 12; i++) {
      eob.addDiagnosis()
          .setDiagnosis(
              new CodeableConcept()
                  .addCoding(new Coding("ICD-10", "A%02d.9".formatted(i), "Diagnosis " + i)))
          .setType(
              List.of(new CodeableConcept().addCoding(new Coding(null, "principal", "Principal"))))
    ...
    for (int i = 0; i < 8; i++) {
      eob.addProcedure()
          .setProcedure(
              new CodeableConcept()
                  .addCoding(new Coding("ICD-10-PCS", "B%02d".formatted(i), "Procedure " + i)))
  ...
  private void printResults(String strategyName, int iterations, long totalMillis) {
    ...
    System.out.printf(
        "Average Latency:      %.4f ms per Bundle%n", ((double) totalMillis / iterations));
    System.out.println("=========================================================");
  }
}
