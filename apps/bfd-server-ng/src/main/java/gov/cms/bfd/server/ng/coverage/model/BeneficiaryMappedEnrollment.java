...
public class BeneficiaryMappedEnrollment {
    ...
    @EmbeddedId
    private BeneficiaryEnrollmentId id;
    ...
    public List<Extension> toFhirExtensions() {
        ...
        var extDrugPlanNumber =
                drugPlanNumber.map(
                        validCode ->
                                new Extension(SystemUrls.EXT_BENE_PBP_NUM_URL)
                                        .setValue(new StringType(validCode)));
                ...
                coverageTypeCode.map(
                        validCode ->
                                new Extension(SystemUrls.EXT_BENE_CVRG_TYPE_CD_URL)
                                        .setValue(
                                                new Coding(SystemUrls.SYS_CVRG_TYPE_CD, validCode, null)));

        return Stream.of(extContractNumber, extDrugPlanNumber, extCoverageTypeCode).flatMap(Optional::stream).toList();
    }
}
