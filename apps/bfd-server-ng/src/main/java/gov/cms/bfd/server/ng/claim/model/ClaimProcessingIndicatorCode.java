...
public sealed interface ClaimProcessingIndicatorCode
   ...
   * @return claim processing indicator code or empty Optional if code is null or blank
   */
  static Optional<ClaimProcessingIndicatorCode> tryFromCode(String code) {
    ...
    return Optional.of(
        Arrays.stream(Valid.values())
            ...
            .map(v -> (ClaimProcessingIndicatorCode) v)
            .findFirst()
  ...
  default Extension toFhir() {
    return new Extension(SystemUrls.EXT_CLM_PRCSG_IND_CD_URL)
        .setValue(new Coding(SystemUrls.SYS_CLM_PRCSG_IND_CD, getCode(), getDisplay()));
  }

  /** Enum for all known, valid codes. */
  @AllArgsConstructor
  @Getter
  enum Valid implements ClaimProcessingIndicatorCode {
    /** A - Allowed. */
    A("A", "Allowed"),
    ...
    /**
     * 15 - MSP cost avoided â€” Workman's Compensation Insurer Voluntary Data Sharing Agreements
     * (WC VDSA) (eff. 4/2006).
     */
    _15(
        "15",
  ...
  record Invalid(String code) implements ClaimProcessingIndicatorCode {
    @Override
