...
public enum ClaimSupplierTypeCode {
  /**
   * 0 - CLINICS, GROUPS, ASSOCIATIONS, INTERVENTION, OR OTHER ENTITIES FOR WHICH THE CARRIER'S OWN
  ...
  /**
   * 1 - PHYSICIANS OR SUPPLIERS BILLING AS SOLO-PRACTIONERS FOR WHOM SOCIAL SECURITY NUMBERS (SSN)
   * ARE SHOWN IN THE PHYSICIAN ID CODE FIELD.
   */
  _1(
      "1",
  ...
  /**
   * 5 - INSTITUTIONAL PROVIDERS AND INDEPENDENT LABORATORIES FOR WHOM EMPLOYER ID NUMBERS ARE USED
   ...
   * @return claim supplier type code
   */
  public static Optional<ClaimSupplierTypeCode> fromCode(String code) {
  ...
  Extension toFhir() {
    ...
    return new Extension(SystemUrls.EXT_PROVIDER_TYPE_CD_URL)
        .setValue(new Coding(SystemUrls.SYS_PROVIDER_TYPE_CD, code, display));
  }
}
