...
public class ClaimRenderingProvider {
    ...
    Optional<Practitioner> toFhirPractitioner(Optional<Integer> claimLineNum) {
        ...
        if (npiNumber.isEmpty() && taxNumber.isEmpty()
                && participatingIndicatorCode.isEmpty() && typeCode.isEmpty()) {
        ...
        npiNumber.ifPresent(s -> practitioner.addIdentifier(
                new Identifier()
                        ...
                        .setValue(s)));
        taxNumber.ifPresent(s -> practitioner.addIdentifier(
                new Identifier()
                        .setType(
                                new CodeableConcept(
        ...
        participatingIndicatorCode.ifPresent(s -> practitioner.addExtension(
                new Extension()
                        .setUrl(SystemUrls.BLUE_BUTTON_STRUCTURE_DEFINITION_PROVIDER_PARTICIPATING_CODE)
                        .setValue(new Coding()
                                .setCode(s)
                                .setSystem(SystemUrls.BLUE_BUTTON_CODE_SYSTEM_PROVIDER_PARTICIPATING_CODE))));
        typeCode.ifPresent(s-> practitioner.addExtension(
                new Extension()
