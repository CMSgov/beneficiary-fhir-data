...
@Embeddable
@Getter
public class ClaimRecordTypeInstitutional {
  ...
  public Optional<Reference> toFhirReference(ClaimTypeCode claimTypeCode) {
    return Stream.of(
            claimRecordTypeCode.map(ClaimRecordTypeCode::getDisplay), claimTypeCode.toDisplay())
  ...
  public Stream<ExplanationOfBenefit.SupportingInformationComponent> toFhir(
    ...
    return claimRecordTypeCode.stream().map(code -> code.toFhir(supportingInfoFactory));
  }
  ...
  public Optional<String> getPartDisplay() {
    return claimRecordTypeCode.map(ClaimRecordTypeCode::getPartDisplay);
  }
}
