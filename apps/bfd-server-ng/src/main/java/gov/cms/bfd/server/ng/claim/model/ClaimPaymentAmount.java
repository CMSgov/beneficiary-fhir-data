...
class ClaimPaymentAmount {
  ...
  ExplanationOfBenefit.PaymentComponent toFhir() {
      var payment = new ExplanationOfBenefit.PaymentComponent().setAmount(USD.toFhir(claimPaymentAmount));
      claimPaymentDate.ifPresent(
              d -> {
                  payment.setDate(DateUtil.toDate(d));
              });
    return payment;
  }
}
