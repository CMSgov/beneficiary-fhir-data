...
public enum ClaimPricingLocalityCode {
   ...
   * @return claim pricing locality code
   */
  public static Optional<ClaimPricingLocalityCode> fromCode(String code) {
    return Arrays.stream(values()).filter(v -> v.code.equals(code)).findFirst();
  }

  Extension toFhir() {
    return new Extension(SystemUrls.EXT_CLM_PRCNG_LCLTY_CD_URL)
        .setValue(new Coding(SystemUrls.SYS_CLM_PRCNG_LCLTY_CD, code, display));
  }
}
