...
public class ClaimAnsiSignature {
  ...
  private Optional<RevenueCenterAnsiReasonCode> revenueCenterAnsiReasonCode4;
    ...
    private Optional<ExplanationOfBenefit.AdjudicationComponent> toAdjudicationComponent(
        ...
        var groupCodings = groupOpt
                .flatMap(RevenueCenterAnsiGroupCode::toFhirCodings)
                .orElse(List.of());
        ...
        var codings = Stream.concat(
                groupCodings.stream(),
                reasonOpt.flatMap(RevenueCenterAnsiReasonCode::toFhirCodings).orElse(List.of()).stream()
        ).toList();
