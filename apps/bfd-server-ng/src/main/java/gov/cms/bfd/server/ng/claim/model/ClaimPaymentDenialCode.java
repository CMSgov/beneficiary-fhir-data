...
public sealed interface ClaimPaymentDenialCode
   ...
   * @return claim payment denial code or empty Optional if code is null or blank
   */
  static Optional<ClaimPaymentDenialCode> tryFromCode(String code) {
    ...
    return Optional.of(
        Arrays.stream(Valid.values())
            ...
            .findFirst()
            .map(v -> (ClaimPaymentDenialCode) v)
  ...
  @AllArgsConstructor
  @Getter
  enum Valid implements ClaimPaymentDenialCode {
    ...
    /**
     * U - MSP COST AVOIDED - HEALTH MAINTENANCE ORGANIZATION (HMO) RATE CELL ADJUSTMENT (EFF.
     * 7/96).
     */
    U(
        "U",
  ...
  record Invalid(String code) implements ClaimPaymentDenialCode {
    @Override
