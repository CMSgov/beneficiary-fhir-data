...
YAML_DICTS_DIR = "/Users/lewellync/Projects/BFD/git/beneficiary-fhir-data/apps/bfd-server-ng/src/main/resources/dictionary-support-files"

CLASS_RE = re.compile(
    r"(?P<type>@Entity|@MappedSuperclass|@Embeddable)[\s\S]*?class\s+(?P<name>\w+)(?:\s+(extends|implements)\s+(?P<parent>\w+))?"
)
...
def parse_yaml_map(yaml_dir: str) -> dict:
    """Maps uppercase database column names to their basis profiles"""
    mappings = {}
                ...
                for entry in yaml.safe_load(f) or []:
                    if source_col := entry.get("sourceColumn"):
                        mappings[source_col.upper()] = (
                            entry.get("profiles") or DEFAULT_PROFILES
                        )
...
def parse_java_graph(src_dir: Path) -> dict:
    ...
    for file_path in src_dir.rglob("*.java"):
        ...
        graph[name] = {
            ...
            "embeddeds": [
                {"type": e[0], "java_var": e[1]} for e in EMBEDDED_RE.findall(content)
            ],
        }
    return graph
...
def print_tree(
    ...
    for col in node["columns"]:
        ...
        print(
            f"{indent}├── [Column] {col['db_col']} ({col['java_var']}) ──► [{', '.join(profiles)}]"
        )
