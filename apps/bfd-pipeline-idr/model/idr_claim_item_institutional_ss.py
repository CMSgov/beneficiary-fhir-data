...
class IdrClaimItemInstitutionalSs(IdrBaseModel):
    ...
    def fetch_query(partition: LoadPartition, start_time: datetime, load_mode: LoadMode) -> str:
        ...
        if load_mode == LoadMode.IDR:
            clm_rlt_cond_cd_agg = f"""ARRAY_AGG({rlt_cond}.clm_rlt_cond_cd) 
                WITHIN GROUP (ORDER BY {rlt_cond}.clm_rlt_cond_sgntr_sqnc_num)"""
        else:
            clm_rlt_cond_cd_agg = f"""ARRAY_AGG({rlt_cond}.clm_rlt_cond_cd 
                ORDER BY {rlt_cond}.clm_rlt_cond_sgntr_sqnc_num)"""
                ...
                claim_line_fiss_bnft_svg AS {not_materialized} (
                    SELECT
                        ...
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 1
                            THEN clm_bnft_svg_ansi_grp_cd END) AS clm_bnft_svg_ansi_grp_1_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 1
                            THEN clm_bnft_svg_ansi_rsn_cd END) AS clm_bnft_svg_ansi_rsn_1_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 2
                            THEN clm_bnft_svg_ansi_grp_cd END) AS clm_bnft_svg_ansi_grp_2_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 2
                            THEN clm_bnft_svg_ansi_rsn_cd END) AS clm_bnft_svg_ansi_rsn_2_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 3
                            THEN clm_bnft_svg_ansi_grp_cd END) AS clm_bnft_svg_ansi_grp_3_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 3
                            THEN clm_bnft_svg_ansi_rsn_cd END) AS clm_bnft_svg_ansi_rsn_3_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 4
                            THEN clm_bnft_svg_ansi_grp_cd END) AS clm_bnft_svg_ansi_grp_4_cd,
                        MAX(CASE WHEN clm_line_bnft_svg_sqnc_num = 4
                            THEN clm_bnft_svg_ansi_rsn_cd END) AS clm_bnft_svg_ansi_rsn_4_cd,
