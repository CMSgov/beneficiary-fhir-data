...
public final class ServerProcess implements AutoCloseable {
  ...
  public ServerProcess(Path warPath, JvmDebugOptions jvmDebugOptions) {
    ...
    try {
      Awaitility.await().atMost(Durations.ONE_MINUTE).until(() -> hasJettyStarted(appRunConsumer));
      Awaitility.await().atMost(Durations.ONE_MINUTE).until(() -> hasWarStarted(appRunConsumer));
