...
public final class PartDEventTransformerTest {
  ...
  @Test
  public void transformSampleARecord2() throws FHIRException {
    List<Object> parsedRecords =
        ServerTestUtils.parseData(Arrays.asList(StaticRifResourceGroup.SAMPLE_A.getResources()));
    PartDEvent claim =
        parsedRecords.stream()
    ...
    ExplanationOfBenefit eob = PartDEventTransformer.transform(new MetricRegistry(), claim);
    assertPartDMatches(claim, eob, "06", "06", TransformerConstants.CODING_UPIN);
  }
  ...
  static void assertPartDMatches(
    ...
    Assert.assertEquals(ServiceProviderIdQualiferCode, claim.getServiceProviderIdQualiferCode());
    Assert.assertEquals(PrescriberIdQualifierCode, claim.getPrescriberIdQualifierCode());
    ...
    TransformerTestUtils.assertReferenceEquals(
        Coding, claim.getServiceProviderId(), eob.getOrganization());
    TransformerTestUtils.assertReferenceEquals(
        Coding, claim.getServiceProviderId(), eob.getFacility());
