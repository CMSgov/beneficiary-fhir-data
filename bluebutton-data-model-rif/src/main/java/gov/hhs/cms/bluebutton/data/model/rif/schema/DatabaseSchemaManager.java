...
public final class DatabaseSchemaManager {
	...
	/**
	 * <strong>WARNING:</strong> This method should never be run against a
	 * production database that is in-service. Any queries against the database
	 * will take over ten minutes to complete. This method is only intended for
	 * (very careful) use when initially loading an empty database. Drops all
	...
	public static void dropIndexes(DataSource dataSource) {
		...
		source = new PlaceholderReplacer(createScriptPlaceholdersMap(dataSource), "${", "}")
	...
	/**
	 * @param dataSource
	 *            the {@link DataSource} that the replacements will be used for
	 * @return the {@link Map} of key-value replacements to use for
	 *         {@link Flyway#setPlaceholders(Map)}
	 */
