locals {
  ...
  request_default_tags_map = merge(
    { for k, v in local.tags : "aws:RequestTag/${k}" => v }
  )
  request_default_tags_no_null = merge(
    { for k, _ in local.tags : "aws:RequestTag/${k}" => false }
  )
  resource_default_tags_map = merge(
    { for k, v in local.tags : "aws:ResourceTag/${k}" => v }
  )
  resource_default_tags_no_null = merge(
    { for k, _ in local.tags : "aws:ResourceTag/${k}" => false }
  )
}

# Role BB2 Insights engineers will assume to manage their resources
...
resource "aws_iam_role" "dev" {
  name = "bfd-insights-bb2-developer"
...
resource "aws_iam_policy" "dev" {
  name        = "bfd-insights-bb2-developer-policy"
  description = "ABAC policy allowing developers to maintain BB2 related Insights resources."
...
resource "aws_iam_policy" "terraform" {
  name        = "bfd-insights-bb2-terraform-policy"
  description = "Allow BB2 developers to apply terraform changes to the BB2 Insights project."
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::bfd-tf-state/bfd-insights/bb2/terraform.tfstate"
        ]
      },
      {
        ...
        Resource = "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/bb2/global/sensitive/insights/*"
      }
    ]
  })
}
...
resource "aws_iam_group" "dev" {
  name = "bfd-insights-bb2-developers"
}

resource "aws_iam_group_policy" "this" {
  name  = "bfd-insights-bb2-developers-assume-role"
