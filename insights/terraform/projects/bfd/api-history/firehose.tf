...
resource "aws_kinesis_firehose_delivery_stream" "bfd-firehose" {
  ...
  extended_s3_configuration {
    ...
    error_output_prefix = "databases/bfd/${local.environment}_api_requests_errors/!{firehose:error-output-type}/year=!{timestamp:yyyy}/month=!{timestamp:MM}/day=!{timestamp:dd}/"
    kms_key_arn         = data.aws_kms_key.kms_key.arn
    prefix              = "databases/bfd/${local.environment}_api_requests/year=!{timestamp:yyyy}/month=!{timestamp:MM}/day=!{timestamp:dd}/"
    ...
    data_format_conversion_configuration {
      ...
      schema_configuration {
        ...
        region        = local.region
        role_arn      = aws_iam_role.firehose_role.arn
        table_name    = "bfd-${local.environment}-api-requests"
        version_id    = "LATEST"
      }
    }
...
resource "aws_cloudwatch_log_subscription_filter" "bfd-access-log-subscription" {
  name            = "bfd-${local.environment}-access-log-subscription"
