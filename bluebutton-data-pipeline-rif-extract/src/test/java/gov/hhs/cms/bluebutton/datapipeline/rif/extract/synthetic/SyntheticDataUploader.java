...
/**
 * Small one-off helper app that uploads the synthetic data to the appropriate
 * place in S3.
 */
public final class SyntheticDataUploader {
	...
	/**
	 * Pushes the synthetic data up to S3, replacing any versions that are
	 * already there.
	 * 
	 * @param args
	 *            (not used)
	...
	public static void main(String[] args) throws Exception {
		...
		uploadSyntheticData(s3Client, TestDataSetLocation.SYNTHETIC_DATA.getS3KeyPrefix().replace("-fixed", ""),
	 ...
	 *            the {@link Function} that returns the {@link Path} to upload
	 *            from, for a given {@link SyntheticDataFile}
	 * @throws MalformedURLException
	 *             Any {@link MalformedURLException}s encountered will be
	 *             bubbled up.
	 */
	private static void uploadSyntheticData(AmazonS3 s3Client, String s3KeyPrefix,
		...
		PutObjectRequest manifestRequest = DataSetTestUtilities.createPutRequest(bucket, s3KeyPrefix,
				manifest);
		...
		for (SyntheticDataFile syntheticDataFile : SyntheticDataFile.values()) {
			DataSetManifestEntry manifestEntry = manifest.getEntries().stream()
					.filter(e -> e.getName()
							.equals(syntheticDataPathGrabber.apply(syntheticDataFile).getFileName().toString()))
					.findFirst().get();

			PutObjectRequest fileRequest = DataSetTestUtilities.createPutRequest(bucket, s3KeyPrefix,
					manifest, manifestEntry, syntheticDataPathGrabber.apply(syntheticDataFile).toUri().toURL());
